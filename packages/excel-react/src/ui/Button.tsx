...
export function Button({ variant = 'outline', size = 'md', active, disabled, onClick, children }: Props) {
  const cls = [
    'flex items-center justify-center rounded-md px-3 text-sm transition-colors duration-120',
    size === 'sm' ? 'h-7' : 'h-8',
    disabled ? 'cursor-default text-ink-disabled' : active && variant === 'ghost' ? 'bg-primary-soft text-primary' : VARIANT[variant],
